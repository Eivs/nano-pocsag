...
// transmit message on 433.900 Mhz using si4463-based "RF24" ISM modules
...
void setup()
{
  ...
  //  rf24.setFrequency(433.9000, 0.00);
  rf24.setFrequency(152.6500, 0.050); // subtract 30 Khz for crystal correction
                                      // this correction-value will differ per device.
                                      // to be determined experimental

  // set to 512 bps FSK, 4.5 Khz deviation (POCSAG 512)
  // rf24.setModemConfig(RH_RF24::FSK_Rb_512Fd4_5 );
...
void loop()
{
  ...
  if (state == -2)
  {
    ...
    // ISM band: 135.050 to 175.800
    if ((newfreq >= 135.050F) && (newfreq <= 175.8F))
    {
      Serial.print("switching to new frequency: ");
      Serial.println(newfreq);
      freq = newfreq;
      rf24.setFrequency(freq, 0.05); // set frequency, AfcPullinRange not used (receive-only)
    }
    else
    {
      Serial.print("Error: invalid frequency (should be 135.050-175.800) ");
      Serial.println(newfreq);
    }; // end if
